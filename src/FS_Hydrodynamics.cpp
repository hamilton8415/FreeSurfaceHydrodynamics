  ...
  : m_L{1.0}, m_grav{9.80665}, m_rho{1025}
{
  ...
  {
    { // Plot Added Mass and Radiation Damping Values
      ...
      for (int i = 0; i < 6; i++) {
        for (int j = i; j < 6; j++) {
          ...
          if ((am_norm > 1e-6) || (dmp_norm > 1e-6)) {
            ...
            if (i == j) {
              gp << "set term qt title 'Added Mass and Radiation Damping (" << i + 1 << "," <<
                  j + 1 << ")\n";
  ...
  /// \brief Returns radiation damping at specified frequency for specified mode (i,j)
  ///  Interpolates from tabulated WAMIT data, omega must be greater than or equal
  ///  to zero.
  double FS_HydroDynamics::RadiationDamping(double omega, int i, int j)
  {
    if (omega < 0) {
      std::cout <<
          "ERROR:  FS_Hydrodyanmics::RadiationDamping:  omega must be great than zero" <<
          std::endl;
    }
  ...
  ///  Interpolates from tabulated WAMIT data, omega must be greater than or equal
  ///  to zero.
  Eigen::Matrix<double, 6, 6> FS_HydroDynamics::RadiationDamping(double omega)
  {
    Eigen::Matrix<double, 6, 6> B;

    for (int i = 0; i < 6; i++) {
      for (int j = 0; j < 6; j++) {
        B(i, j) = RadiationDamping(omega, i, j);
      }
    }
  ...
  Eigen::VectorXd FS_HydroDynamics::GravityForce(Eigen::VectorXd x)
  {
    ...
    F_G.setZero();
    F_G(2) = -this->m_grav * this->M(0, 0);  // F_G(0:2) = (0,0,-mg)
    F_G(3) = this->m_grav * this->M(0, 0) * (this->COG(2) * sin(x(3)));
    F_G(4) = this->m_grav * this->M(0, 0) * (this->COG(2) * sin(x(4)));
    return F_G;
  }
...
//  }
  ...
  Eigen::Matrix<std::complex<double>, 6, 1> FS_HydroDynamics::ComplexAmplitude(double omega)
  {
    ...
    auto B = RadiationDamping(omega);
    auto Chi = WaveExcitingForceComponents(omega);
    Eigen::Matrix<std::complex<double>, 6, 6>  LinDamping;
  LinDamping = m_b.asDiagonal();
  Eigen::Matrix<std::complex<double>, 6, 6> C;
  C = -pow(omega,2)*(A+this->M) + i*omega*B + i*LinDamping + this->c;
  return C.inverse()*Chi;
  }

  std::complex<double> FS_HydroDynamics::ComplexAmplitude(double omega, int j)
  {
    auto A = AddedMass(omega);
    auto B = RadiationDamping(omega);
    auto Chi = WaveExcitingForceComponents(omega);
    ...
    Xi = Chi(j) /
    (-pow(omega, 2) * (A(j, j) + this->M(j, j)) + i * omega * B(j, j) + i * m_b(j) + this->c(j, j));
  return Xi;
}
